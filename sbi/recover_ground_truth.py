...
``deprecated_code/tester_scripts/data_factory.py`` varies four parameters but
writes only two of them to ``data/pattern_stats.npy``. The mean cluster radius
``cr`` and the radius spread ``rb`` were never saved.
...
# Mirrors data_factory.py exactly. Changing any of these breaks the replay.
FACTORY_SEED = 42
FACTORY_N_SIMS = 1000
FACTORY_PCP = 0.1
...
# Column layout of data/pattern_stats.npy, from data_factory.py.
STATS_COLUMNS = {
    "pcp_measured": 0,
    "pcp_true": 1,
    "pcp_percent_error": 2,
...
def main():
    ...
    if not gate_passed:
        raise SystemExit(
            ...
            "data_factory.py's RNG usage has changed and cr/rb cannot be trusted. "
    ...
    provenance = {
        ...
        "warning": (
            ...
            "here. Any change to the RNG draws in data_factory.py invalidates the "
