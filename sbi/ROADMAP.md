...
Measured over all 1,000 patterns by `sbi/recover_ground_truth.py`, so
...
Implemented by `sbi/recover_ground_truth.py`.
...
| Path | Contents |
| --- | --- |
| `sbi/ROADMAP.md` | this document |
| `sbi/recover_ground_truth.py` | Stage 0 |
| `sbi/ground_truth/` | persisted θ and per-pattern descriptors |
| `sbi/extract_features.py` | Stage 1 (to be written) |
| `sbi/fit_posterior.py` | Stage 2 (to be written) |
| `sbi/validate_posterior.py` | Stage 3 (to be written) |
...
1. **Stage 0** — θ recovered, verified and persisted.
   `sbi/recover_ground_truth.py`, output in `sbi/ground_truth/`. Gate passed
