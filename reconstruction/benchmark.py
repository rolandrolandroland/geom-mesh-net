...
Every stage of ``reconstruction/ROADMAP.md`` reads patterns and observations
...
protocol. ``reconstruction/freeze_benchmark.py`` writes them, with checksums, to
``reconstruction/benchmark/``; changing any of them invalidates every stored
result.
"""
...
DATA_DIR = Path("data_random_centres")   # the benchmark: random cluster centres
LATTICE_DATA_DIR = Path("data")          # lattice centres, for Stage 3's comparison only
THETA_PATH = Path("inference/ground_truth/theta.npy")
BENCHMARK_DIR = Path("reconstruction/benchmark")
ORACLE_DIR = Path("reconstruction/results/oracle")
