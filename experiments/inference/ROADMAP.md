...
`scripts/generate_data.py` varies exactly four
...
This recovery depends on `scripts/generate_data.py` remaining byte-identical in its RNG
...
`geom_mesh_net/statistics/paper_spatial_features.py`, compressing 216,000
...
`experiments/neural_field/EXPERIMENTAL_METHODOLOGY_01.md` had to prespecify advancement
criteria by hand.
...
Measured over all 1,000 patterns by `experiments/inference/recover_ground_truth.py`, so
...
`experiments/inference/screen_features.py` screens the extracted features with ridge
...
Implemented by `experiments/inference/recover_ground_truth.py`.
...
established in `statistics/feature_cache.py`.
...
`docs/guides/clustersim_todo.md` already asks "which features capture the
...
`experiments/neural_field/methodology_01_results` and
`experiments/neural_field/global_paper_feature_validation`.
...
Note that `experiments/neural_field/global_paper_feature_validation/validation_config.json`
...
repeated random splits (`experiments/inference/compare_feature_sets.py`), with the two
...
(`experiments/inference/posterior/feature_set_comparison.json`, 25 splits; ± is the sd across
splits):
...
`experiments/inference/generate_shared_upp.py` re-simulated all 1,000 parameter vectors from
...
(`experiments/inference/posterior/shared_upp_comparison.json`):
...
The folder was named `inference/`, not `sbi/`, because a local `sbi/` directory
shadows the `sbi` pip package that Stage 2 was planned to depend on:
`from sbi.inference import NPE` fails outright with `PYTHONPATH=.` set. The name
was changed before any imports were written against it. The track now lives in
`experiments/inference/`.

| Path | Contents |
| --- | --- |
| `experiments/inference/ROADMAP.md` | this document |
| `experiments/inference/recover_ground_truth.py` | Stage 0 |
| `experiments/inference/ground_truth/` | persisted θ and per-pattern descriptors |
| `experiments/inference/extract_features.py` | Stage 1 |
| `experiments/inference/features/` | cached features (`.npz` gitignored, `.json` tracked) |
| `experiments/inference/screen_features.py` | ridge screen of feature informativeness |
| `experiments/inference/ablate_features.py` | Stage 4 |
| `geom_mesh_net/inference/flow.py` | conditional autoregressive flow, written here not imported |
| `experiments/inference/fit_posterior.py` | Stage 2 |
| `experiments/inference/posterior/` | fitted flow, test posterior samples, metadata |
| `experiments/inference/validate_posterior.py` | Stage 3 |
...
- Whether to regenerate `experiments/neural_field/global_paper_feature_validation/`, whose
