...
of every pattern in ``data/`` -- read from
``experiments/inference/ground_truth/theta.npy`` -- so that the only difference
between the two datasets is whether the underlying points are shared. Any change
in recoverability is attributable to that.
...
Usage
-----
    python -m experiments.inference.generate_shared_upp
    python -m experiments.inference.generate_shared_upp --limit 50 --output-dir /tmp/shared
"""
...
# Mirrors scripts/generate_data.py so that point sharing is the only change.
SIDE = 60
OPP_SIDE = 30
INTENSITY = 1
PCP = 0.1
...
def main():
    ...
    parser.add_argument("--output-dir", type=Path, default=paths.SHARED_UPP_DIR)
    parser.add_argument("--theta", type=Path, default=paths.THETA_PATH)
    parser.add_argument("--limit", type=int, default=None)
    args = parser.parse_args()

    if args.output_dir.resolve() == paths.DATA_DIR.resolve():
    ...
    (args.output_dir / "provenance.json").write_text(json.dumps({
        "generated_by": "experiments/inference/generate_shared_upp.py",
