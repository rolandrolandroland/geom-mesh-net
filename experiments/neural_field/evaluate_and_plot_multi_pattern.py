...
# Import core modules
from geom_mesh_net import neural as dl
from geom_mesh_net import paths
from geom_mesh_net.viz import clusters as cv

CHECKPOINTS_DIR = paths.NEURAL_FIELD_DIR / "checkpoints"

# Ensure figures directory exists
figures_dir = os.path.join(CHECKPOINTS_DIR, "figures")
...
dataset = dl.LoadData(
    ...
    data_prefix=f"{paths.DATA_DIR}/",
    params_prefix=f"{paths.DATA_DIR}/",
    marks="all",
...
for pattern_idx in range(size):
    ...
    for model_name, model in models.items():
        ...
        for epoch in range(100, 1001, 100):
            ...
            checkpoint_path = os.path.join(CHECKPOINTS_DIR, f"pattern_{pattern_idx}", model_name, f"epoch_{epoch}.pth")
