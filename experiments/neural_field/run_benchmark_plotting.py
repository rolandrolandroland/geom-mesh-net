...
from geom_mesh_net.viz import benchmarks as cb

CHECKPOINTS_DIR = paths.NEURAL_FIELD_DIR / "checkpoints"


def run_benchmark_report_suite(checkpoints_dir=CHECKPOINTS_DIR, output_dir=CHECKPOINTS_DIR / "figures", num_patterns=5):
    """
    Main orchestration routine to load metrics from checkpoints and execute all plotting functions
    in viz.benchmarks for every pattern, ensuring unique filenames with pattern identifiers.
    """
    print("=" * 65)
...
if __name__ == "__main__":
    run_benchmark_report_suite(
        ...
        output_dir=CHECKPOINTS_DIR / "figures",
        num_patterns=5
    )