"""Stage 0: build the replay oracle and decide Gate 0 (experiments/reconstruction/ROADMAP.md, Stage 0).

For each pattern, ``fields.oracle.replay_oracle`` gives every atom's guest
...
to ``experiments/reconstruction/results/oracle/`` for later stages.

Usage
-----
    python -m experiments.reconstruction.stage0_oracle
    python -m experiments.reconstruction.stage0_oracle --workers 6 --replays 1000
"""
...
def main():
    ...
    parser.add_argument("--output", type=Path, default=paths.RECONSTRUCTION_DIR / "results" / "stage0_gate.json")
    ...
    with Pool(args.workers) as pool:
        for name, data_dir, gated in (("benchmark", bm.DATA_DIR, True), ("lattice", bm.LATTICE_DATA_DIR, False)):
            ...
            summary["data_dir"] = paths.repo_relative(data_dir)
            summary["gated"] = gated
