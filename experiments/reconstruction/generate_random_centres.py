...
box, its spacing fixed by the parameters (``experiments/reconstruction/ROADMAP.md`` section
...
``experiments/inference/ground_truth/theta.npy``, and every other setting of
``scripts/generate_data.py``. It changes one thing: the overlying pattern is uniform
...
-----
    ...
    python -m experiments.reconstruction.generate_random_centres --limit 20 --output-dir /tmp/rc
"""
...
# Mirrors scripts/generate_data.py so that centre placement is the only change.
SIDE = 60
OPP_SIDE = 30
INTENSITY = 1
PCP = 0.1
...
def main():
    ...
    parser.add_argument("--output-dir", type=Path, default=paths.RANDOM_CENTRES_DIR)
    parser.add_argument("--theta", type=Path, default=paths.THETA_PATH)
    parser.add_argument("--reference-dir", type=Path, default=paths.DATA_DIR,
                        help="lattice dataset whose cluster counts are compared")
    parser.add_argument("--limit", type=int, default=None)
    args = parser.parse_args()

    if args.output_dir.resolve() in {paths.DATA_DIR.resolve(), paths.SHARED_UPP_DIR.resolve()}:
    ...
    provenance = {
        "generated_by": "experiments/reconstruction/generate_random_centres.py",
        "design": "same parameter vectors and settings as data/; overlying pattern uniform random, not a lattice",
        "theta_source": paths.repo_relative(args.theta),
        ...
        "cluster_count_differences_from_reference": {
            "reference_dir": paths.repo_relative(args.reference_dir),
