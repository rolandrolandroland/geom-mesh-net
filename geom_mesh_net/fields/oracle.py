...
realised labels are only one random draw from it. ``density_grid.
generate_density_grid`` only approximates it, and gets it wrong inside clusters
(``experiments/reconstruction/ROADMAP.md`` section 3.2). It can be computed
exactly, up to a Monte Carlo error this module drives down, because clustersim's
labelling is a known random procedure applied to stored inputs:
...
``"Gaussian_decay"`` (linear weights, the default in ``scripts/generate_data.py``)
or ``"constant"``.
"""
